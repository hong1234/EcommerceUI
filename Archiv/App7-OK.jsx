...
function Detail({ prodId, setProdId }) {
  ...
  const postData = async (prod, fdata) => {
    const cartItemDTO = {
      ...
      price: prod.price,
      color: fdata.color,
      sizeLong: fdata.lsize,
      sizeWidth: fdata.wsize,
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={fieldStyle}>
          <label htmlFor="wsize">Width size in cm:</label>
          <br />
          <input
            type="number"
            id="wsize"
            {...register("wsize", {
              required: " width size is required",
              min: {
                value: 50,
                message: "Your width size must be at least 50 cm",
              },
              max: {
                value: 100,
                message: "Your width size must be less than 100 cm",
              },
            })}
            // className={getEditorStyle(errors.wsize)}
          />
          {errors.wsize && (
            <span className="error-message">{errors.wsize.message}</span>
          )}
        </div>
        ...
        <div className={fieldStyle}>
          ...
          <input
            ...
            // className={getEditorStyle(errors.pieces)}
          />
          {errors.pieces && (
    ...
    return (
      <>
        <div className="sidebar-bottom tile">
          ...
          {/* <br /> */}
          <p>
            <b>No Products of category {`"${category}"`}</b>
          </p>
        </div>
      </>
    );
  ...
  return (
    <>
      <div className="sidebar-bottom tile">
        ...
        <br /> */}
        <p>
          <b>Products of category {`"${category}"`}</b>
        </p>
        {/* <br /> */}
        <ul>
          {totalPages.length > 1 ? (
            totalPages.map((pa) => (
              <button>
                <Link key={pa} to={`/shop/products/${category}/${pa}`}>
                  Page-{pa}
                </Link>
              </button>
            ))
      ...
      <ul>
        {state.products.map((item) => (
          <li key={item.id}>
            ...
            <br /> */}
            {item.title}{" "}
            {`(${item.sizeLong}, ${item.sizeWidth}, ${item.color})`} |
